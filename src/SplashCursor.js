...
/* Fluid-simulation cursor trail (WebGL).
   Based on Pavel Dobryakov's "WebGL Fluid Simulation" (MIT).
   Renders on a transparent, pointer-events-none canvas overlay.
   Colors are biased to the site palette: Aegean blues + Attic gold. */
...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    /* Brand palette: mostly Aegean blues, occasionally gold. */
    function generateColor() {
      const hue =
        Math.random() < 0.72
          ? 0.52 + Math.random() * 0.13
  ...
  return <canvas ref={canvasRef} className="splash-cursor" />;
}
